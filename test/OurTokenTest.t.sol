...
contract OurTokenTest is Test {
    ...
    function testAllowanceWorks() public {
        ...
        // bob approves alex to spend token on his behalf
        vm.prank(bob);
        ourToken.approve(alex, initialAllowance);

        uint256 transferAmount = 500;

        vm.prank(alex);
        ourToken.transferFrom(bob, alex, transferAmount);

        assertEq(ourToken.balanceOf(alex), transferAmount);
